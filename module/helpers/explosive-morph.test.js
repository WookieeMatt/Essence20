...
describe("activateExplosiveMorph", () => {
  beforeEach(() => {
    canvas.tokens.setTargets.mockReset();
    canvas.tokens.placeables = [];
  });

  test("targets every nearby enemy and rolls Technology vs. Evasion", async () => {
    ...
    expect(actor._dice.rollSkill).toHaveBeenCalledWith(
      expect.objectContaining({
        skill: 'technology', essence: 'smarts', defenseType: 'evasion', isExplosiveMorph: true,
      }),
      actor,
    );
  });
